...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        Qvalue=0
        f_values = self.featExtractor.getFeatures(state, action)
        for feature in f_values:
          Qvalue+= f_values[feature]*self.weights[feature]
        return Qvalue
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        # difference = reward + gama*V(s') - Q(s,a)

        value = -inf
        for nextAction in self.getLegalActions(nextState):
          if self.getQValue(nextState,nextAction)>value:
            value =self.getQValue(nextState,nextAction)
        if value == -inf:
          value =0    
      
        diff = reward + self.discount*value - self.getQValue(state,action)

        f_values = self.featExtractor.getFeatures(state, action)
        for feature in f_values:
          self.weights[feature] +=self.alpha*diff*f_values[feature]
