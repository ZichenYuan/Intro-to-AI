...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            obs = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            self.beliefs[ghostPosition] *=obs
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for key in newPosDist:
                newBelief[key]+=newPosDist[key]*self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        x =len(self.legalPositions)
        
        for i in range (self.numParticles):
            position =self.legalPositions[i%x]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(self.numParticles):
            self.particles[i] = weightDistribution.sample()
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            result[particle]+= 1.0/self.numParticles
        return result
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        legalPos=list(itertools.product(self.legalPositions,self.legalPositions))
        random.shuffle(legalPos)
        size = len(legalPos)
        for i in range(self.numParticles):
            position=legalPos[i % size]
            self.particles.append(position)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                weight_ith_ghost = self.getObservationProb(observation[i], pacmanPos,particle[i],jailPos)
                weight*=weight_ith_ghost #update the weight with the observation of the ith ghost
